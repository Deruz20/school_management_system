...
import { mockEnrollments, mockClasses } from './mock-data';
...
export function SearchFilterBar({
  ...
  const { mode, studentIds, classId, section, term, phase, curriculum } = filterState;

  const filteredClasses = useMemo(() => {
    if (section === 'all') return mockClasses;
    return mockClasses.filter(c => c.section_type === section);
  }, [section]);

  const filteredEnrollments = useMemo(() => {
    let list = mockEnrollments;
    if (section !== 'all') list = list.filter(e => e.section_type === section);
    if (classId) list = list.filter(e => {
    ...
    return list;
  }, [section, classId, studentSearch]);

  const canGenerate = !!term && !!phase && (
    mode === 'class' ? !!classId : studentIds.length > 0
  );
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' as const }}>
              ...
              <FilterGroup label="Mode">
                <SegmentedPair
                  ...
                  value={mode}
                  onChange={(v) => onChange({ mode: v as 'class' | 'individual', studentIds: [], classId: '' })}
                  activeColor="#047857"
                />
              </FilterGroup>
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                  {Object.entries(SECTION_LABELS).map(([val, lbl]) => (
                    <PillButton
                      key={val}
                      active={section === val}
                      onClick={() => onChange({ section: val as FilterState['section'], classId: '' })}
                      activeColor="#047857"
                      size="sm"
                    >
                      {lbl}
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' as const }}>
              ...
              {mode === 'class' ? (
                <FilterGroup label="Select Class">
                    ...
                    {filteredClasses.map((cls) => (
                      <motion.button
                        ...
                        style={{
                          ...
                          cursor: 'pointer',
                          background: classId === cls.id ? '#047857' : 'white',
                          color: classId === cls.id ? 'white' : '#374151',
                          border: `1.5px solid ${classId === cls.id ? '#047857' : 'rgba(0,0,0,0.12)'}`,
                        }}
                        whileHover={{ scale: 1.04 }}
                        whileTap={{ scale: 0.95 }}
                      >
                        ...
                        <span style={{
                          ...
                          background: classId === cls.id ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.07)',
                ...
                <FilterGroup label={`Select Students${studentIds.length > 0 ? ` (${studentIds.length} selected)` : ''}`}>
                  ...
                  <div style={{
                    ...
                    gap: 4, maxHeight: 130, overflowY: 'auto',
                    scrollbarWidth: 'thin',
                  }}>
